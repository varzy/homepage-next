...
function parseKotobaFrontmatter(filePath: string): KotobaPost | null {
  const fileContent = FileUtils.readFileSync(filePath);
  if (!fileContent) return null;

  try {
    const { data } = matter(fileContent) as { data: KotobaFrontmatterData; content: string };
    return {
      page_id: data.page_id || '',
      title: data.title || '',
      status: data.status || '',
